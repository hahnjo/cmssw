...
#include "RNTupleEventOutputFile.h"
...
namespace edm {
  ...
  class RNTupleEventOutputModule : public one::OutputModule<> {
  public:
    explicit RNTupleEventOutputModule(ParameterSet const& pset);
    ~RNTupleEventOutputModule() final;
  ...
  private:
    ...
    std::unique_ptr<RNTupleEventOutputFile> file_;
  ...
  RNTupleEventOutputModule::RNTupleEventOutputModule(ParameterSet const& pset)
  ...
  void RNTupleEventOutputModule::openFile(FileBlock const& fb) {
    RNTupleEventOutputFile::Config conf;
    ...
    assert(reg_);
    file_ = std::make_unique<RNTupleEventOutputFile>(fileName_, fb, keptProducts(), conf, reg_->anyProductProduced());
  }

  void RNTupleEventOutputModule::initialRegistry(edm::ProductRegistry const& iReg) {
    reg_ = std::make_unique<ProductRegistry>(iReg.productList());
  }

  void RNTupleEventOutputModule::reallyCloseFile() {
    if (file_) {
      assert(reg_);
  ...
  RNTupleEventOutputModule::~RNTupleEventOutputModule() = default;

  void RNTupleEventOutputModule::write(EventForOutput const& e) { file_->write(e); }

  void RNTupleEventOutputModule::writeLuminosityBlock(LuminosityBlockForOutput const& iLumi) {
    file_->writeLuminosityBlock(iLumi);
  }

  void RNTupleEventOutputModule::writeRun(RunForOutput const& iRun) { file_->writeRun(iRun); }

  void RNTupleEventOutputModule::fillDescriptions(ConfigurationDescriptions& descriptions) {
...
using edm::RNTupleEventOutputModule;
DEFINE_FWK_MODULE(RNTupleEventOutputModule);
