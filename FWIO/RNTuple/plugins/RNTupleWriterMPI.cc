...
public:
  RPageSinkMPIAggregator(const RNTupleWriterMPI::Config &config)
    ...
    ROOT::Experimental::RNTupleWriteOptions options = config.fOptions;
    ...
    while (processes > 0) {
      ...
      while (nColumns > 0) {
        ...
        for (const auto &pageInfo : pageRange.fPageInfos) {
          ...
          assert(pageInfo.fLocator.GetPosition<std::uint64_t>() == offset);
          offset += bufferSize;
          if (ptr) {
            ptr += bufferSize;
          }
...
public:
  ...
  void InitImpl(RNTupleModel &model) final {
    ...
    for (auto &f : fieldZero) {
      ...
      ROOT::Experimental::Internal::CallConnectPageSinkOnField(f, *this);
    }
    ...
    for (int i = 0; i < fSize; i++) {
      ...
      for (DescriptorId_t clusterId = 0; clusterId < nClusters; clusterId++) {
        ...
        for (const auto &columnRange :
          ...
          offsets.push_back(
              firstPage.fLocator.GetPosition<std::uint64_t>());
          break;
        }
      }
    }
...
RNTupleWriterMPI::Recreate(Config config, int root, MPI_Comm comm) {
  ...
  return ROOT::Experimental::Internal::CreateRNTupleWriter(std::move(config.fModel),
                                             std::move(sink));
}
