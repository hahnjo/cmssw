...
namespace edm {
  ...
  class RNTupleMPIOutputModule : public one::OutputModule<> {
  public:
    explicit RNTupleMPIOutputModule(ParameterSet const& pset);
    ~RNTupleMPIOutputModule() final;
  ...
  private:
    ...
    std::unique_ptr<RNTupleMPIOutputFile> file_;
  ...
  RNTupleMPIOutputModule::RNTupleMPIOutputModule(ParameterSet const& pset)
  ...
  void RNTupleMPIOutputModule::openFile(FileBlock const& fb) {
    RNTupleMPIOutputFile::Config conf;
    ...
    assert(reg_);
    file_ = std::make_unique<RNTupleMPIOutputFile>(fileName_, fb, keptProducts(), conf, reg_->anyProductProduced());
  }

  void RNTupleMPIOutputModule::initialRegistry(edm::ProductRegistry const& iReg) {
    reg_ = std::make_unique<ProductRegistry>(iReg.productList());
  }

  void RNTupleMPIOutputModule::reallyCloseFile() {
    if (file_) {
      assert(reg_);
  ...
  RNTupleMPIOutputModule::~RNTupleMPIOutputModule() = default;

  void RNTupleMPIOutputModule::write(EventForOutput const& e) { file_->write(e); }

  void RNTupleMPIOutputModule::writeLuminosityBlock(LuminosityBlockForOutput const& iLumi) {
    file_->writeLuminosityBlock(iLumi);
  }

  void RNTupleMPIOutputModule::writeRun(RunForOutput const& iRun) { file_->writeRun(iRun); }

  void RNTupleMPIOutputModule::fillDescriptions(ConfigurationDescriptions& descriptions) {
...
using edm::RNTupleMPIOutputModule;
DEFINE_FWK_MODULE(RNTupleMPIOutputModule);
