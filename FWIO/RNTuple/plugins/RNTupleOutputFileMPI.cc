#include "RNTupleOutputFileMPI.h"
...
namespace {
  ...
  RNTupleWriterMPI::Config createRNTupleWriterMPIConfig() {
...
using namespace ROOT;
namespace edm {
  RNTupleOutputFileMPI::RNTupleOutputFileMPI(std::string const& iFileName,
    ...
    if (rank == kRoot) {
      file_.reset(TFile::Open(fileName_.c_str(), "recreate", ""));
    }
...
explicitly pass the correct variable to `SetColumnRepresentatives`).
  ...
  void RNTupleOutputFileMPI::setupDataProducts(SelectedProducts const& iProducts,
  ...
  std::vector<RNTupleOutputFileMPI::Product> RNTupleOutputFileMPI::associateDataProducts(SelectedProducts const& iProducts,
  ...
  std::unique_ptr<RNTupleModel> RNTupleOutputFileMPI::setupCommonModels(SelectedProducts const& iProducts,
  ...
  void RNTupleOutputFileMPI::setupRuns(SelectedProducts const& iProducts, Config const& iConfig) {
    std::string kRunAuxName = "RunAuxiliary";
    {
      ...
      config.fNTupleName = "Runs";
      config.fStorage = fileName_;
      config.fOptions = writeOptions;
      runs_ = RNTupleWriterMPI::Append(std::move(config), file_.get(), kRoot, MPI_COMM_WORLD);
    }
    products_[InRun] = associateDataProducts(iProducts, runs_->GetModel());
    runAuxField_ = runs_->GetModel().GetToken(kRunAuxName);
  }
  void RNTupleOutputFileMPI::setupLumis(SelectedProducts const& iProducts, Config const& iConfig) {
    std::string kLumiAuxName = "LuminosityBlockAuxiliary";
    {
      ...
      auto config = createRNTupleWriterMPIConfig();
      config.fModel = std::move(model);
      config.fNTupleName = "LuminosityBlocks";
      config.fStorage = fileName_;
      config.fOptions = writeOptions;
      lumis_ = RNTupleWriterMPI::Append(std::move(config), file_.get(), kRoot, MPI_COMM_WORLD);
    }
    products_[InLumi] = associateDataProducts(iProducts, lumis_->GetModel());
    lumiAuxField_ = lumis_->GetModel().GetToken(kLumiAuxName);
  }
  void RNTupleOutputFileMPI::setupEvents(SelectedProducts const& iProducts,
                                      Config const& iConfig,
                                      bool anyProductProduced) {
    ...
    std::string kBranchListName = "BranchListIndexes";
    {
      ...
      auto config = createRNTupleWriterMPIConfig();
      config.fModel = std::move(model);
      config.fNTupleName = "Events";
      config.fStorage = fileName_;
      config.fOptions = writeOptions;
      events_ = RNTupleWriterMPI::Append(std::move(config), file_.get(), kRoot, MPI_COMM_WORLD);
    }
  ...
  void RNTupleOutputFileMPI::setupPSets(Config const& iConfig) {
    auto model = ROOT::RNTupleModel::CreateBare();
    {
      auto field =
    ...
    auto config = createRNTupleWriterMPIConfig();
    config.fModel = std::move(model);
    config.fNTupleName = "ParameterSets";
    config.fStorage = fileName_;
    config.fOptions = writeOptions;
    parameterSets_ = RNTupleWriterMPI::Append(std::move(config), file_.get(), kRoot, MPI_COMM_WORLD);
  }

  void RNTupleOutputFileMPI::fillPSets() {
  ...
  void RNTupleOutputFileMPI::setupParentage(Config const& iConfig) {
    auto model = ROOT::RNTupleModel::CreateBare();
    {
    ...
    auto config = createRNTupleWriterMPIConfig();
    config.fModel = std::move(model);
    config.fNTupleName = "Parentage";
    config.fStorage = fileName_;
    config.fOptions = writeOptions;
    parentage_ = RNTupleWriterMPI::Append(std::move(config), file_.get(), kRoot, MPI_COMM_WORLD);
  }
  void RNTupleOutputFileMPI::fillParentage() {
  ...
  void RNTupleOutputFileMPI::setupMetaData(Config const& iConfig) {
    auto model = ROOT::RNTupleModel::CreateBare();
    {
    ...
    auto config = createRNTupleWriterMPIConfig();
    config.fModel = std::move(model);
    config.fNTupleName = "MetaData";
    config.fStorage = fileName_;
    config.fOptions = writeOptions;
    metaData_ = RNTupleWriterMPI::Append(std::move(config), file_.get(), kRoot, MPI_COMM_WORLD);
  }

  void RNTupleOutputFileMPI::fillMetaData(BranchIDLists const& iBranchIDLists,
  ...
  void RNTupleOutputFileMPI::openFile(FileBlock const& fb) {
  ...
  void RNTupleOutputFileMPI::reallyCloseFile(BranchIDLists const& iBranchIDLists,
                                          ThinnedAssociationsHelper const& iThinnedHelper,
                                          ProductRegistry const& iReg) {
    fillPSets();
    fillParentage();
    ...
    events_->CommitCluster();
    runs_->CommitCluster();
    lumis_->CommitCluster();
    parameterSets_->CommitCluster();
    parentage_->CommitCluster();
    metaData_->CommitCluster();

    events_.reset();
    runs_.reset();
    lumis_.reset();
    parameterSets_.reset();
    parentage_.reset();
    metaData_.reset();
  }

  RNTupleOutputFileMPI::~RNTupleOutputFileMPI() {}

  std::vector<std::unique_ptr<WrapperBase>> RNTupleOutputFileMPI::writeDataProducts(std::vector<Product> const& iProducts,
  ...
  std::vector<StoredProductProvenance> RNTupleOutputFileMPI::writeDataProductProvenance(
  ...
  bool RNTupleOutputFileMPI::insertProductProvenance(ProductProvenance const& iProv,
    ...
    if (itFound == oToInsert.end()) {
      ...
      if (toStore.parentageIDIndex_ >= parentageIDs_.size()) {
        throw edm::Exception(errors::LogicError)
            << "RNTupleOutputFileMPI::insertProductProvenance\n"
  ...
  void RNTupleOutputFileMPI::insertAncestorsProvenance(ProductProvenance const& iProv,
  ...
  void RNTupleOutputFileMPI::write(EventForOutput const& e) {
    {
  ...
  void RNTupleOutputFileMPI::writeLuminosityBlock(LuminosityBlockForOutput const& iLumi) {
    {
  ...
  void RNTupleOutputFileMPI::writeRun(RunForOutput const& iRun) {
    {
