...
namespace edm {
  ...
  class RNTupleOutputModuleMPI : public one::OutputModule<> {
  public:
    explicit RNTupleOutputModuleMPI(ParameterSet const& pset);
    ~RNTupleOutputModuleMPI() final;
  ...
  private:
    ...
    std::unique_ptr<RNTupleOutputFileMPI> file_;
  ...
  RNTupleOutputModuleMPI::RNTupleOutputModuleMPI(ParameterSet const& pset)
  ...
  void RNTupleOutputModuleMPI::openFile(FileBlock const& fb) {
    RNTupleOutputFileMPI::Config conf;
    ...
    assert(reg_);
    file_ = std::make_unique<RNTupleOutputFileMPI>(fileName_, fb, keptProducts(), conf, reg_->anyProductProduced());
  }

  void RNTupleOutputModuleMPI::initialRegistry(edm::ProductRegistry const& iReg) {
    reg_ = std::make_unique<ProductRegistry>(iReg.productList());
  }

  void RNTupleOutputModuleMPI::reallyCloseFile() {
    if (file_) {
      assert(reg_);
  ...
  RNTupleOutputModuleMPI::~RNTupleOutputModuleMPI() = default;

  void RNTupleOutputModuleMPI::write(EventForOutput const& e) { file_->write(e); }

  void RNTupleOutputModuleMPI::writeLuminosityBlock(LuminosityBlockForOutput const& iLumi) {
    file_->writeLuminosityBlock(iLumi);
  }

  void RNTupleOutputModuleMPI::writeRun(RunForOutput const& iRun) { file_->writeRun(iRun); }

  void RNTupleOutputModuleMPI::fillDescriptions(ConfigurationDescriptions& descriptions) {
...
using edm::RNTupleOutputModuleMPI;
DEFINE_FWK_MODULE(RNTupleOutputModuleMPI);
