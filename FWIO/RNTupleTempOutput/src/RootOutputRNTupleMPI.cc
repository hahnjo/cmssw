...
#include "RootOutputRNTupleMPI.h"
...
namespace edm {
  ...
  RootOutputRNTupleMPI::RootOutputRNTupleMPI(std::shared_ptr<TFile> filePtr,
                                       std::string const& fileName,
                                       BranchType const& branchType,
                                       std::string const& processName)
      : filePtr_(filePtr), fileName_(fileName),
  ...
  void RootOutputRNTupleMPI::fill() {
  ...
  void RootOutputRNTupleMPI::finishInitialization(Config const& config) {
    ...
    auto writerConfig = createRNTupleWriterMPIConfig();
    writerConfig.fModel = std::move(model_);
    writerConfig.fNTupleName = name_;
    writerConfig.fStorage = fileName_;
    writerConfig.fOptions = options;
    writer_ = RNTupleWriterMPI::Append(std::move(writerConfig), filePtr_.get(), kRoot, MPI_COMM_WORLD);
  }
...
explicitly pass the correct variable to `SetColumnRepresentatives`).
  ...
  void RootOutputRNTupleMPI::addField(std::string const& branchName,
  ...
  void RootOutputRNTupleMPI::close() {
