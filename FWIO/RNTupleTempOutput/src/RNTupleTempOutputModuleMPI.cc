#include "FWIO/RNTupleTempOutput/interface/RNTupleTempOutputModuleMPI.h"
...
namespace edm::rntuple_temp {
  inline bool RNTupleTempOutputModuleMPI::SetStreamerForDataProduct::match(std::string const& iBranchName) const {
    return std::regex_match(iBranchName, branch_);
  }

  std::regex RNTupleTempOutputModuleMPI::SetStreamerForDataProduct::convert(
  ...
  namespace {
    std::vector<RNTupleTempOutputModuleMPI::SetStreamerForDataProduct> fromConfig(
        std::vector<edm::ParameterSet> const& iConfig) {
      std::vector<RNTupleTempOutputModuleMPI::SetStreamerForDataProduct> returnValue;
    ...
    std::optional<bool> useStreamer(std::string const& iName,
                                    std::vector<RNTupleTempOutputModuleMPI::SetStreamerForDataProduct> const& iSpecial) {
    ...
    edm::rntuple_temp::RNTupleTempOutputModuleMPI::Optimizations fromConfig(edm::ParameterSet const& iConfig) {
      edm::rntuple_temp::RNTupleTempOutputModuleMPI::Optimizations opts;
...
namespace edm::rntuple_temp {
  RNTupleTempOutputModuleMPI::RNTupleTempOutputModuleMPI(ParameterSet const& pset)
  ...
  void RNTupleTempOutputModuleMPI::beginJob() {}

  void RNTupleTempOutputModuleMPI::initialRegistry(edm::ProductRegistry const& iReg) {
    reg_ = std::make_unique<ProductRegistry>(iReg.productList());
  }

  std::string const& RNTupleTempOutputModuleMPI::currentFileName() const { return rootOutputFile_->fileName(); }

  RNTupleTempOutputModuleMPI::AuxItem::AuxItem() : basketSize_(ProductDescription::invalidBasketSize) {}

  RNTupleTempOutputModuleMPI::OutputItem::OutputItem(ProductDescription const* bd,
  ...
  bool RNTupleTempOutputModuleMPI::AliasForBranch::match(std::string const& iBranchName) const {
    return std::regex_match(iBranchName, branch_);
  }

  std::regex RNTupleTempOutputModuleMPI::AliasForBranch::convert(std::string const& iGlobBranchExpression) const {
    return convertBranchExpression(iGlobBranchExpression);
  }

  void RNTupleTempOutputModuleMPI::fillSelectedItemList(BranchType branchType,
  ...
  void RNTupleTempOutputModuleMPI::beginInputFile(FileBlock const& fb) {
    if (isFileOpen()) {
  ...
  void RNTupleTempOutputModuleMPI::openFile(FileBlock const& fb) {
    if (!isFileOpen()) {
      reallyOpenFile();
      beginInputFile(fb);
    }
  ...
  void RNTupleTempOutputModuleMPI::respondToOpenInputFile(FileBlock const& fb) {
  ...
  void RNTupleTempOutputModuleMPI::respondToCloseInputFile(FileBlock const& fb) {
    if (rootOutputFile_)
      rootOutputFile_->respondToCloseInputFile(fb);
  }

  void RNTupleTempOutputModuleMPI::setProcessesWithSelectedMergeableRunProducts(std::set<std::string> const& processes) {
    processesWithSelectedMergeableRunProducts_.assign(processes.begin(), processes.end());
  }

  RNTupleTempOutputModuleMPI::~RNTupleTempOutputModuleMPI() {}

  void RNTupleTempOutputModuleMPI::write(EventForOutput const& e) {
  ...
  void RNTupleTempOutputModuleMPI::writeLuminosityBlock(LuminosityBlockForOutput const& lb) {
    rootOutputFile_->writeLuminosityBlock(lb);
  }

  void RNTupleTempOutputModuleMPI::writeRun(RunForOutput const& r) {
  ...
  void RNTupleTempOutputModuleMPI::writeProcessBlock(ProcessBlockForOutput const& pb) {
    rootOutputFile_->writeProcessBlock(pb);
  }

  void RNTupleTempOutputModuleMPI::reallyCloseFile() {
    fillDependencyGraph();
  ...
  void RNTupleTempOutputModuleMPI::startEndFile() {}

  void RNTupleTempOutputModuleMPI::writeMetaData() { rootOutputFile_->writeMetaData(*reg_); }

  void RNTupleTempOutputModuleMPI::writeParameterSetRegistry() { rootOutputFile_->writeParameterSetRegistry(); }
  void RNTupleTempOutputModuleMPI::writeParentageRegistry() { rootOutputFile_->writeParentageRegistry(); }
  void RNTupleTempOutputModuleMPI::finishEndFile() {
    rootOutputFile_->finishEndFile();
    rootOutputFile_ = nullptr;
  }  // propagate_const<T> has no reset() function
  void RNTupleTempOutputModuleMPI::doExtrasAfterCloseFile() {}
  bool RNTupleTempOutputModuleMPI::isFileOpen() const { return rootOutputFile_.get() != nullptr; }
  bool RNTupleTempOutputModuleMPI::shouldWeCloseFile() const { return rootOutputFile_->shouldWeCloseFile(); }

  std::pair<std::string, std::string> RNTupleTempOutputModuleMPI::physicalAndLogicalNameForNewFile() {
  ...
  void RNTupleTempOutputModuleMPI::reallyOpenFile() {
    auto names = physicalAndLogicalNameForNewFile();
    rootOutputFile_ = std::make_unique<RootOutputFileMPI>(this,
                                                       names.first,
                                                       names.second,
  ...
  void RNTupleTempOutputModuleMPI::updateBranchParentsForOneBranch(ProductProvenanceRetriever const* provRetriever,
  ...
  void RNTupleTempOutputModuleMPI::updateBranchParents(EventForOutput const& e) {
  ...
  void RNTupleTempOutputModuleMPI::preActionBeforeRunEventAsync(WaitingTaskHolder iTask,
  ...
  void RNTupleTempOutputModuleMPI::fillDependencyGraph() {
  ...
  void RNTupleTempOutputModuleMPI::fillDescription(ParameterSetDescription& desc) {
  ...
  void RNTupleTempOutputModuleMPI::fillDescriptions(ConfigurationDescriptions& descriptions) {
    ParameterSetDescription desc;
    RNTupleTempOutputModuleMPI::fillDescription(desc);
    descriptions.add("edmOutputMPI", desc);
  }
}  // namespace edm::rntuple_temp
