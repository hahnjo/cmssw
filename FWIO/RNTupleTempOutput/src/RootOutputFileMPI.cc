...
#include "FWIO/RNTupleTempOutput/src/RootOutputFileMPI.h"
...
namespace edm::rntuple_temp {
  ...
  RootOutputFileMPI::RootOutputFileMPI(RNTupleTempOutputModuleMPI* om,
      ...
      : file_(fileName),
        ...
        eventRNTuple_(filePtr(), file_, InEvent),
        lumiRNTuple_(filePtr(), file_, InLumi),
        runRNTuple_(filePtr(), file_, InRun),
    ...
    for (auto const& processName : processesWithProcessBlockProducts) {
      processBlockRNTuples_.emplace_back(std::make_unique<RootOutputRNTupleMPI>(filePtr(), file_, InProcess, processName));
    }
    ...
    if (om_->compressionAlgorithm() == std::string("ZLIB")) {
      ...
      throw Exception(errors::Configuration)
          << "RNTupleTempOutputModuleMPI configured with unknown compression algorithm '" << om_->compressionAlgorithm()
          << "'\n"
    ...
    for (unsigned int i = 0; i < treePointers_.size(); ++i) {
      RootOutputRNTupleMPI* theRNTuple = treePointers_[i];
    ...
    RootOutputRNTupleMPI::Config config;
    config.compressionAlgo =
        om_->compressionAlgorithm() == std::string("LZMA")   ? RootOutputRNTupleMPI::Config::CompressionAlgos::kLZMA
        : om_->compressionAlgorithm() == std::string("ZSTD") ? RootOutputRNTupleMPI::Config::CompressionAlgos::kZSTD
        : om_->compressionAlgorithm() == std::string("LZ4")  ? RootOutputRNTupleMPI::Config::CompressionAlgos::kLZ4
                                                             : RootOutputRNTupleMPI::Config::CompressionAlgos::kZLIB;
    ...
    std::string moduleName = "RNTupleTempOutputModuleMPI";
  ...
  void RootOutputFileMPI::beginInputFile(FileBlock const& fb, int remainingEvents) {}

  void RootOutputFileMPI::respondToCloseInputFile(FileBlock const&) {
  ...
  bool RootOutputFileMPI::shouldWeCloseFile() const {
    return false;
  }

  void RootOutputFileMPI::writeOne(EventForOutput const& e) {
    // Auxiliary branch
  ...
  void RootOutputFileMPI::writeLuminosityBlock(LuminosityBlockForOutput const& lb) {
    // Auxiliary branch
  ...
  void RootOutputFileMPI::writeRun(RunForOutput const& r) {
    // Auxiliary branch
  ...
  void RootOutputFileMPI::writeProcessBlock(ProcessBlockForOutput const& pb) {
  ...
  namespace {
    ROOT::RNTupleWriteOptions optionsFrom(RNTupleTempOutputModuleMPI const& iOM) {
  ...
  void RootOutputFileMPI::writeMetaData(ProductRegistry const& iReg) {
    ...
    auto config = createRNTupleWriterMPIConfig();
    config.fModel = std::move(model);
    config.fNTupleName = poolNames::metaDataTreeName();
    config.fStorage = file_;
    config.fOptions = optionsFrom(*om_);
    auto metaData =
        RNTupleWriterMPI::Append(std::move(config), filePtr_.get(), kRoot, MPI_COMM_WORLD);
  ...
  void RootOutputFileMPI::writeParentageRegistry() {
    auto model = ROOT::RNTupleModel::CreateBare();
    model->AddField(ROOT::RFieldBase::Create(poolNames::parentageBranchName(), "edm::Parentage").Unwrap());

    auto config = createRNTupleWriterMPIConfig();
    config.fModel = std::move(model);
    config.fNTupleName = poolNames::parentageTreeName();
    config.fStorage = file_;
    config.fOptions = optionsFrom(*om_);
  ...
  void RootOutputFileMPI::writeFileIdentifier(ROOT::REntry& rentry) {
    rentry.BindRawPtr(poolNames::fileIdentifierBranchName(), &fid_);
  }

  void RootOutputFileMPI::writeIndexIntoFile(ROOT::REntry& rentry) {
    indexIntoFile_.sortVector_Run_Or_Lumi_Entries();
    rentry.BindRawPtr(poolNames::indexIntoFileBranchName(), &indexIntoFile_);
  }

  void RootOutputFileMPI::writeStoredMergeableRunProductMetadata(ROOT::REntry& rentry) {
  ...
  void RootOutputFileMPI::writeProcessHistoryRegistry(ROOT::REntry& rentry) {
  ...
  void RootOutputFileMPI::writeBranchIDListRegistry(ROOT::REntry& rentry) {
    rentry.BindRawPtr(poolNames::branchIDListBranchName(), const_cast<BranchIDLists*>(om_->branchIDLists()));
  }

  void RootOutputFileMPI::writeThinnedAssociationsHelper(ROOT::REntry& rentry) {
  ...
  void RootOutputFileMPI::writeProductDescriptionRegistry(ROOT::REntry& rentry, ProductRegistry const& iReg) {
  ...
  void RootOutputFileMPI::writeProductDependencies(ROOT::REntry& rentry) {
  ...
  void RootOutputFileMPI::writeProcessBlockHelper(ROOT::REntry& rentry) {
    if (!om_->outputProcessBlockHelper().processesWithProcessBlockProducts().empty()) {
  ...
  void RootOutputFileMPI::writeParameterSetRegistry() {
    auto model = ROOT::RNTupleModel::CreateBare();
    {
      auto field =
    ...
    auto config = createRNTupleWriterMPIConfig();
    config.fModel = std::move(model);
    config.fNTupleName = poolNames::parameterSetsTreeName();
    config.fStorage = file_;
    config.fOptions = optionsFrom(*om_);
    auto parameterSets =
        RNTupleWriterMPI::Append(std::move(config), filePtr_.get(), kRoot, MPI_COMM_WORLD);
  ...
  void RootOutputFileMPI::finishEndFile() {
    ...
    try {
      ...
      e.addContext("Calling RootOutputFileMPI::finishEndFile() while closing " + file_);
      e.addAdditionalInfo("While calling " + std::string(status) + std::string(value));
      throw;
    }
  }

  void RootOutputFileMPI::insertAncestors(ProductProvenance const& iGetParents,
                                       ProductProvenanceRetriever const* iMapper,
                                       bool produced,
                                       std::set<BranchID> const& iProducedIDs,
                                       std::set<StoredProductProvenance>& oToFill) {
    assert(om_->dropMetaData() != RNTupleTempOutputModuleMPI::DropAll);
    assert(produced || om_->dropMetaData() != RNTupleTempOutputModuleMPI::DropPrior);
    if (om_->dropMetaData() == RNTupleTempOutputModuleMPI::DropDroppedPrior && !produced)
      return;
    ...
    for (auto const& parentID : parentIDs) {
      ...
      if (info) {
        if (om_->dropMetaData() == RNTupleTempOutputModuleMPI::DropNone ||
  ...
  void RootOutputFileMPI::fillBranches(BranchType const& branchType,
    ...
    bool const doProvenance =
        (productProvenanceVecPtr != nullptr) && (om_->dropMetaData() != RNTupleTempOutputModuleMPI::DropAll);
    bool const keepProvenanceForPrior = doProvenance && om_->dropMetaData() != RNTupleTempOutputModuleMPI::DropPrior;
    ...
    if (doProvenance && branchType == InEvent && om_->dropMetaData() != RNTupleTempOutputModuleMPI::DropNone) {
  ...
  bool RootOutputFileMPI::insertProductProvenance(const edm::ProductProvenance& iProv,
    ...
    if (itFound == oToInsert.end()) {
      ...
      if (toStore.parentageIDIndex_ >= parentageIDs_.size()) {
        throw edm::Exception(errors::LogicError)
            << "RootOutputFileMPI::insertProductProvenance\n"
